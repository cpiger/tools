...
:: Do FZF Setting Here
REM  set FZF_DEFAULT_COMMAND=fd -a -j 4
REM  set FZF_DEFAULT_OPTS=--prompt="=>" --border --inline-info 
...
:WITH_START
REM  start "winpty" %1 "%SELECTION%"
REM  cd  "%SELECTION%" >nul 2>nul
dir/ad "%SELECTION%" >nul 2>nul && cd "%SELECTION%" || start "winpty" %1 "%SELECTION%"
goto :END
